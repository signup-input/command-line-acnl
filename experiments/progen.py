...
-----------------------------------------------------"""
        ...
        story = f'''Chapter One
My name is {mc.firstname} {mc.lastname}. My dream is to {mc.motives}.
Me and {sc.firstname} are...
----------------------
"{sc.firstname}!" I {random.choice(["shout","scream","call"])}. "{random.choice(["Wait up!", "I said I'm sorry!", "Are you okay?", "You were serious?"])}"
{sc.pronouns()[0].capitalize()} is far ahead of me, {random.choice([f"weaving through {random.choice([f'the sea of people', 'the crowd'])}", "sprinting", "ramming into people " + random.choice(["like a maniac", f"like {sc.pronouns()[0]+' has gone *insane*'}"])])}.
...
I {random.choice(["numbly continue to walk and make it to","keep walking and arrive at", "see"])} {random.choice(["""another platform and lean on the guardrail.
I look down at the track below""", "a metal bench and throw myself on it", 'a large clock with eye-slicing Roman numerals. "How grim." I realize'])}, {random.choice(['sighing', 'sobbing', 'scrubbing my tears away', 'willing myself not to cry. Tough people dont cry...'])}.
My train {random.choice(["left just minutes ago","is gone"])}{random.choice([" and I have no way to buy", ". There is no point in buying", " and why buy"])} another ticket.
